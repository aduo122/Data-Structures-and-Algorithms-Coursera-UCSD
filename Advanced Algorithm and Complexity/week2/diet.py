...
def run_solve(n, m, A, b, c):
    ...
    for i in equationGroups:
        ...
        for j in i:
            ...
            m2.append(b[j])
        try:
            x = np.linalg.solve(np.array(m1), np.array(m2))
        except:
            continue
        prerequisite = True
...
def solve_check(n, m, A, b, c, ansx):
    numOfEquations = len(A)
    equationGroups = list(itertools.combinations([i for i in range(n + m)], m))
...
bCheck = b + [10 ** 9]
ansx = run_solve(n, m, A, b, c)
anst = 0
if len(ansx) == 0:
    anst = -1
elif ansx != solve_check(n + 1, m, ACheck, bCheck, c, ansx):
    anst = 1
if anst == -1:
    print("No solution")
if anst == 0:
