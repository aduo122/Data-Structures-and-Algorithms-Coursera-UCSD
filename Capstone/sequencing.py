...
class AssemblingErrorFree:
    def __init__(self):
        reads = self.readData()
        print(reads)
        gene = self.assembly(reads)
        print(gene)
    ...
    def longestOverlap(self, text, patterns, k=12):
    ...
    def assembly(self, reads):
        ...
        while True:
            ...
            if 1 == len(reads):
                break
            del reads[currInd]
            currInd, overlap = self.longestOverlap(currRead + '$', reads)
            gene += reads[currInd][overlap:]
        currInd, overlap = self.longestOverlap(reads[0] + '$', [firstRead])
        if overlap > 0:
            return gene[0:-overlap]
        else:
            return gene
